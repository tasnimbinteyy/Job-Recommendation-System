...
export async function GET() {
  try {
    ...
    if (userSkills.length === 0) {
      feedback.push({
        type: "warning",
        message: "Add skills to your profile to improve your score.",
      });
    } else if (userSkills.length < 4) {
      feedback.push({
        type: "warning",
        message: `You have ${userSkills.length} skill(s). Aim for at least 6-8 skills.`,
      });
    } else {
      feedback.push({
        type: "success",
        message: `Good skill count: ${userSkills.length} skills listed.`,
      });
    }

    // 2. Market demand score (max 40 points) — how many of user's skills are in demand
    const demandedSkills = userSkills.filter(
      (s) => (skillFrequency[s] ?? 0) > 0,
    );
    const demandScore =
      totalJobs > 0
        ? Math.min(
            (demandedSkills.length / Math.max(userSkills.length, 1)) * 40,
            40,
          )
        : 0;
    score += demandScore;
    ...
    if (demandedSkills.length === 0 && userSkills.length > 0) {
      feedback.push({
        type: "warning",
        message:
          "None of your skills match current job requirements. Update your skills.",
      });
      ...
      feedback.push({
        type: "success",
        message: `High-demand skills you have: ${topDemanded.join(", ")}.`,
      });
    }
    ...
    if (user.experience && user.experience.trim().length > 10) {
      ...
      feedback.push({
        type: "success",
        message: "Experience section is filled in.",
      });
    } else {
      feedback.push({
        type: "info",
        message: "Add your experience to boost your score by 20 points.",
      });
    }
